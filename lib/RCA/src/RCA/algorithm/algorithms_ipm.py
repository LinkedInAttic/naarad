...
class expAvgDetector(object):
  ...
  def __init__(self, data, smoothing_factor=None, lag_window_size=None):
    self.data = data
    self.smoothing_factor = smoothing_factor if smoothing_factor > 0 else DEFAULT_EMA_SMOTHING_FACTOR
  ...
  def _compute_anom_score(self, lag_window_points, point):
    stdev = numpy.std(lag_window_points)
...
class BitmapDetector(object):
  ...
  """
  def __init__(self, data, precision=4, lag_window_size=None, chunk_size=None):
    self.data = data
    self.precision = precision if precision is not None and precision > 0 else 4
    self.lag_window_size = lag_window_size if lag_window_size else int(self.data_length*DEFAULT_BITMAP_LEADING_WINDOW_SIZE_PCT)
    self.chunk_size = chunk_size if chunk_size is not None and chunk_size > 0 else DEFAULT_BITMAP_CHUNK_SIZE
  ...
  def detect_anom_use_both_lag_and_future_window(self):
    ...
    self.future_window_size = self.data_length*DEFAULT_BITMAP_LEADING_WINDOW_SIZE_PCT
    for i in range(chunk_size+1, sax_len):
  ...
  def _compute_derivative(self):
    ...
    for (i, [timestamp, value]) in enumerate(self.data):
      if i < self.data_length-1:
        ...
        t2 = utils.to_epoch(next_point[0])
        v1 = value
        v2 = next_point[1]
        td = t2-t1
